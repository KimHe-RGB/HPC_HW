...
// when we update red points at (m,n), we must make sure that all surrounding black points ARE NOT updated
// when we update black points at (m,n), we must make sure that all surrounding red points ARE updated
void gs2D_omp(double** grid, int n, double h, int iters) {
    // f(x, y) ≡ 1; normally this should be read from a function value matrix
    double fij = 1.0; 
    ...
    for (int k = 1; k < iters; k++)
    {
        ...
        // make sure all red points are updated at this point
        #pragma omp barrier
...
int main() {
    ...
    gs2D_seq(gridVal, N+2, h, 20);
    printf("sequential Gauss-Siedel = %fs\n", omp_get_wtime() - tt);

    tt = omp_get_wtime();
    gs2D_omp(gridVal2, N+2, h, 20);
    printf("parallel Gauss-Siedel = %fs, %d threads, N = %d\n", omp_get_wtime() - tt, NUM_OF_THREADS, N);
